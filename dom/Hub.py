...
<div style = 'color:#0000FF; width: 800px; font-size: 14px' id = "activity{num}"></div>
'''

import TechElement as hub

class Hub_basic:
    def __init__(self, divName = 'all_things_hubs', suffix='_hub'):
        self.hubInfo = hub
        self.hubdiv = document.getElementById('all_things_hubs')
            ...
            try:
                ...
                for i in range(50):
                    if self.info:  # waits for the info_response to arrive and get parsed
                        await self.feed_rate(100)
                        if self.info["GroupID"] in [514, 515]: 
                            label = 'sensor'
                        else:
                            label = 'motor'
                        document.getElementById('var_1').value = label
                        return
                    #print('waiting')
    ...
    def device_message(self, data, verbose = False):
        ...
        while data:
            ...
            if ID in self.hubInfo.DEVICE_MESSAGE_MAP:
                ...
                print(f"Unknown device ID: {ID} >> {data}")
                break
        return messages
    ...
    def my_callback(self, characteristic, data):
        ...
        if ID == 1:
            ...
            if int(info['Firmware']['build']) < 63:
                window.alert('You need to update your firmware')
            self.info = info
